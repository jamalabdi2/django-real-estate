...
try:
    ...
    if (
        ...
        or os.path.basename(sys.argv[0]) == "buildprefixdata.py"
    ):
        prnt(
            "Failed to import generated data (but OK as during autogeneration)",
            file=sys.stderr,
        )
...
def name_for_valid_number(numobj, lang, script=None, region=None):
    ...
    """
    return _prefix_description_for_number(
        CARRIER_DATA, CARRIER_LONGEST_PREFIX, numobj, lang, script, region
    )
...
def _is_mobile(ntype):
    ...
    return (
        ntype == PhoneNumberType.MOBILE
        or ntype == PhoneNumberType.FIXED_LINE_OR_MOBILE
        or ntype == PhoneNumberType.PAGER
    )
