...
_EMPTY_METADATA = PhoneMetadata(
    id=unicod(""), international_prefix=unicod("NA"), register=False
)
...
_ELIGIBLE_FORMAT_PATTERN = re.compile(
    unicod("[")
    + _VALID_PUNCTUATION
    + unicod("]*")
    + unicod("\\\\1")
...
class AsYouTypeFormatter(object):
        ...
        while ii < len(self._possible_formats):
            ...
            if self._create_formatting_template(number_format):
                ...
                if number_format.national_prefix_formatting_rule is None:
                    ...
                    self._should_add_space_after_national_prefix = bool(
                        _NATIONAL_PREFIX_SEPARATORS_PATTERN.search(
    ...
    def _get_available_formats(self, leading_digits):
        ...
        is_international_number = (
            self._is_complete_number and len(self._extracted_national_prefix) == 0
        )
        if (
            is_international_number
            and len(self._current_metadata.intl_number_format) > 0
        ):
        ...
        for this_format in format_list:
            ...
            # prefix.
            if (
                len(self._extracted_national_prefix) > 0
                and _formatting_rule_has_first_group_only(
                    this_format.national_prefix_formatting_rule
                )
                and not this_format.national_prefix_optional_when_formatting
                and not (this_format.domestic_carrier_code_formatting_rule is not None)
            ):
                ...
                len(self._extracted_national_prefix) == 0
                and not self._is_complete_number
                and not _formatting_rule_has_first_group_only(
                    this_format.national_prefix_formatting_rule
                )
                and not this_format.national_prefix_optional_when_formatting
            ):
    ...
    def _narrow_down_possible_formats(self, leading_digits):
        index_of_leading_digits_pattern = (
            len(leading_digits) - _MIN_LEADING_DIGITS_LENGTH
        )
        ii = 0
        while ii < len(self._possible_formats):
            ...
            if len(num_format.leading_digits_pattern) == 0:
                ...
                continue
            last_leading_digits_pattern = min(
                index_of_leading_digits_pattern,
                len(num_format.leading_digits_pattern) - 1,
            )
            leading_digits_pattern = re.compile(
                num_format.leading_digits_pattern[last_leading_digits_pattern]
            )
    ...
    def input_digit(self, next_char, remember_position=False):
            ...
            if self._input_has_formatting:
                ...
                if self._attempt_to_extract_ccc():
                    self._current_output = (
                        self._attempt_to_choose_pattern_with_prefix_extracted()
                    )
                    return self._current_output
        ...
        if self._is_expecting_country_calling_code:
            if self._attempt_to_extract_ccc():
                self._is_expecting_country_calling_code = False
            self._current_output = (
                self._prefix_before_national_number + self._national_number
            )
            return self._current_output

        if (
            len(self._possible_formats) > 0
            ...
            if self._able_to_format:
                self._current_output = self._append_national_number(
    ...
    def _able_to_extract_longer_ndd(self):
        if len(self._extracted_national_prefix) > 0:
            ...
            self._national_number = (
                self._extracted_national_prefix + self._national_number
            )
            ...
            index_of_previous_ndd = self._prefix_before_national_number.rfind(
                self._extracted_national_prefix
            )
            self._prefix_before_national_number = self._prefix_before_national_number[
                :index_of_previous_ndd
            ]
        return (
            self._extracted_national_prefix
            != self._remove_national_prefix_from_national_number()
    ...
    def _is_digit_or_leading_plus_sign(self, next_char):
        return next_char.isdigit() or (
            len(self._accrued_input) == 1 and fullmatch(_PLUS_CHARS_PATTERN, next_char)
        )
        ...
        for number_format in self._possible_formats:
            ...
            if fullmatch(num_re, self._national_number):
                ...
                formatted_number = re.sub(
                    num_re, number_format.format, self._national_number
                )
                ...
                formatted_number_digits_only = normalize_diallable_chars_only(
                    full_output
                )
                if (
    ...
    def get_remembered_position(self):
        ...
        while (
            accrued_input_index < self._position_to_remember
            and current_output_index < len(self._current_output)
        ):
            if (
                self._accrued_input_without_formatting[accrued_input_index]
    ...
    def _append_national_number(self, national_number):
        ...
        if (
            ...
            and prefix_before_nn_len > 0
            and self._prefix_before_national_number[-1]
    ...
    def _input_accrued_national_number(self):
        ...
        if length_of_national_number > 0:
            ...
            for ii in range(length_of_national_number):
                temp_national_number = self._input_digit_helper(
                    self._national_number[ii]
                )
            if self._able_to_format:
    ...
    def _is_nanpa_number_with_national_prefix(self):
        ...
        return (
            self._current_metadata.country_code == 1
            and self._national_number[0] == "1"
            and self._national_number[1] != "0"
            and self._national_number[1] != "1"
        )
    ...
    def _remove_national_prefix_from_national_number(self):
        ...
        if self._is_nanpa_number_with_national_prefix():
            ...
            self._prefix_before_national_number += (
                unicod("1") + _SEPARATOR_BEFORE_NATIONAL_NUMBER
            )
    ...
    def _attempt_to_extract_idd(self):
        ...
        default_country.
        """
        international_prefix = re.compile(
            unicod("\\")
            + _PLUS_SIGN
            + unicod("|")
        ...
        if idd_match:
            ...
            self._national_number = self._accrued_input_without_formatting[
                start_of_country_calling_code:
            ]
            self._prefix_before_national_number = (
                self._accrued_input_without_formatting[:start_of_country_calling_code]
            )
    ...
    def _attempt_to_extract_ccc(self):
        ...
        if new_region_code == REGION_CODE_FOR_NON_GEO_ENTITY:
            self._current_metadata = PhoneMetadata.metadata_for_nongeo_region(
                country_code
            )
    ...
    def _input_digit_helper(self, next_char):
        ...
        # an IDD or NDD.
        digit_match = _DIGIT_PATTERN.search(
            self._formatting_template, self._last_match_position
        )
        if digit_match:
            ...
            temp_template = re.sub(
                _DIGIT_PATTERN, next_char, self._formatting_template, count=1
            )
            self._formatting_template = (
                temp_template + self._formatting_template[len(temp_template) :]
            )
