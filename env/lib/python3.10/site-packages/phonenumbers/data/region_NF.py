...
PHONE_METADATA_NF = PhoneMetadata(
    id="NF",
    country_code=672,
    ...
    fixed_line=PhoneNumberDesc(
        national_number_pattern="(?:1(?:06|17|28|39)|3[0-2]\\d)\\d{3}",
        example_number="106609",
        possible_length=(6,),
        possible_length_local_only=(5,),
    ),
    mobile=PhoneNumberDesc(
        national_number_pattern="(?:14|3[58])\\d{4}",
    ...
    number_format=[
        NumberFormat(
            pattern="(\\d{2})(\\d{4})",
            format="\\1 \\2",
            leading_digits_pattern=["1[0-3]"],
        ),
        NumberFormat(
            pattern="(\\d)(\\d{5})", format="\\1 \\2", leading_digits_pattern=["[13]"]
        ),
    ],
)
